...
erg=ERGController(A,B,[0 1],0,F,G); %Creation of an instance of ERGsys
erg.addStateConstraint([1 0]',42); %Adding state constraints
erg.addInputConstraint('max',100); %Adding input constraints
erg.addInputConstraint('min',-100);
kappa=1000; %Gain
simout=sim('sim_model.slx','StopTime',num2str(Tsim),'Solver','ode23s','AbsTol','1e-9','RelTol','1e-7'); %Simulate
%% PLOTTING
